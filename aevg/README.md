...
render live — every backend op is wired, no skip-stubs remain. The
`aevg/example_aevg.ae` demo renders an SVG string (rect / circle /
line / filled+stroked cubic path / text) to a live GTK4 window, and
`example_raster.ae` blits a `render_as_raster` buffer. The one
remaining raster gap is *pipeline auto-routing*: the shape factories
don't yet delegate to `render_as_raster` on `filter=`/`clip-path=`
attrs. **Total: 28 modules, 937 assertions, all passing in Phase 0**
(pure-Aether, no display); plus both demos link + smoke-build under
Linux/GTK in CI.
