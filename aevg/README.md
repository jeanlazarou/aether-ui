...
| aevg_gtk_backend | (new — adapter to aether_ui canvas) | `aevg_gtk_backend.ae` | build-linked via `aevg/example_aevg.ae` (ci Phase 1) | ✅ The real-display adapter. `gtk_backend_new(canvas_id)` wraps an `aether_ui` canvas id in a dispatch table whose closure forwards each op to the live canvas primitives: **circle** → `canvas_arc` + `canvas_fill` (and a second arc + `canvas_stroke`); **rect** → `canvas_fill_rect` + a closed 4-segment stroke outline; **line** → `begin_path`/`move`/`line`/`stroke`; **path** → replay the mapped polyline (`canvas_begin_path` + `move_to`/`line_to`/`close_path`) then fill and/or stroke; **text** → `canvas_fill_text` at baseline. Recovers circle center/radius from the factory's bbox opts (`cx=(x+x2)/2`, `r=(x2-x)/2`). Colors parsed via `rasterize.parse_color_to_rgba` (0..255) and scaled to the canvas's 0..1 floats (`int / 255.0` promotes to float in Aether). **Deferred (logged, not silent):** `canvas_raster` (needs a pixel-blit primitive the canvas API lacks). |
...
canvas-space polylines), and text all render live; only raster
(filter/clipPath blit) is still deferred. The `aevg/example_aevg.ae`
demo renders an SVG string — including a filled+stroked cubic path —
to a live GTK4 window. **Total: 27 modules, 933 assertions, all
passing in Phase 0** (pure-Aether, no display); plus the demo links
+ smoke-builds under Linux/GTK in CI.
