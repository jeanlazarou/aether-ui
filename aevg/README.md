...
| gradient fill | `grammar-shapes.ts` `path()` `resolveGradientFill`/`buildFillGradient` + `grammar-rendering.ts` | `grammar_shapes` gradient emit + `aevg_gtk_backend.apply_gradient` + C `canvas_fill_linear/radial_gradient` (GTK4/AppKit/Win32) | `test_gradient_fill.ae` (14 asserts) | ✅ Linear + radial gradient fills, all three platforms. `shape_path` detects `fill=url(#id)` against the gradient registry and, via `emit_gradient_opts`, builds a canvas-space descriptor into the backend opts: `gradType` (linear/radial), mapped geometry (`gradX1..`/`gradCX..`), and `gradStops` (`"off,r,g,b,a …"`, rgba 0..255). objectBoundingBox units (SVG default) map 0..1 coords onto the shape's canvas bbox; userSpaceOnUse maps through the viewBox transform. The adapter's `apply_gradient` parses the stops into `floatarr` offsets+rgba (0..1) and calls the C primitive. **C primitives:** GTK `cairo_pattern_create_linear/radial` + `add_color_stop_rgba` + fill; macOS `CGGradientCreateWithColorComponents` + `CGContextDrawLinear/RadialGradient` clipped to the path; Win32 approximates with a 2-stop `GradientFill` over the path bbox (plain GDI has no multi-stop gradient — documented degradation). Stop arrays are `FloatArray*` read via `floatarr_get_unchecked`, copied + owned per command, freed on clear. Gradient is the **path-borne** shape in CVG (rect/circle delegate to path for gradients via the rounded-rect/rotation branch, not yet ported); `shape_path` is the correct placement. Test verifies plain fill keeps `fillColor`, linear emits `gradType=linear` + mapped endpoints + exact stops string, radial emits `gradType=radial` + mapped center/radius. The `example_aevg.ae` demo's path now uses `fill='url(#sky)'` and renders the gradient live. |
...
radial gradient fills render** on all three platforms (`shape_path`
detects `fill=url(#id)`, builds a canvas-space gradient descriptor,
and the adapter drives new `canvas_fill_linear/radial_gradient` C
primitives — cairo patterns on GTK, CGGradient on macOS, a 2-stop
`GradientFill` approximation on Win32). The `aevg/example_aevg.ae`
demo renders an SVG string — rect / circle / line / **gradient-filled
cubic path** / text **and a blur-filtered circle** — to a live GTK4
window through the complete pipeline. **Total: 30 modules, 961
